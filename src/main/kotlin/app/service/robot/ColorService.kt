...
class ColorService : KoinComponent {
    ...
    private fun resolveClosestDistance(detectedColors: HashMap<Color, Array<Mat>>) : HashMap<Color, Array<Color>>
    {
        ...
        for(color in detectedColors.keys)
        {
            for(i in 0 until 9)
            {
                ...
                {
                    distances[color2] = geometryUtils.getDistance(colorUtils.scalarBGR2Lab(KnnClustering(detectedColors[color]!![i], 30)), centerColors[color2]!!)
                }
    ...
    private fun resolveColorNN(whiteMat : Mat, detectedColors: Array<Mat>) : Array<Color>
    {
    ...
    private fun resolveColorNN(whiteLab : Scalar, DetectedColorLab : Scalar) : Color
    {
