...
class PopulationCornerSolver : PopulationSolver() {
    ...
    override fun gradeSequence(cube: Cube, sequence: Array<Movement>): Int {
        ...
        if(cornerService.findCornerByName(cubeExperiment.corners, 'A')!!.isSolved() && edgeService.findEdgeByName(cubeExperiment.edges, 'E')!!.isSolved()) scoreA += 100
        if(cornerService.findCornerByName(cubeExperiment.corners, 'B')!!.isSolved() && edgeService.findEdgeByName(cubeExperiment.edges, 'F')!!.isSolved()) scoreB += 100
        if(cornerService.findCornerByName(cubeExperiment.corners, 'C')!!.isSolved() && edgeService.findEdgeByName(cubeExperiment.edges, 'G')!!.isSolved()) scoreC += 100
        if(cornerService.findCornerByName(cubeExperiment.corners, 'D')!!.isSolved() && edgeService.findEdgeByName(cubeExperiment.edges, 'H')!!.isSolved()) scoreD += 100
